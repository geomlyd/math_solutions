...
\begin{solution}
    ...
    (iii) We've already shown this in part (ii) by the arguments involving the fact that $f$ is piecewise constant.
    What we also know is that $\epsilon_n > 0$, and as such the partial sums of the corresponding series is strictly increasing, meaning that $f(x_i) < f(x_{i+1})$ for all $i$.
    In addition, we showed that $f(x) = f(x_i)$ for $x \in [x_i, x_{i+1})$.
    Therefore, to make $f$ strictly increasing it suffices to make it strictly increasing inside each interval $[x_i, x_{i+1})$, strictly increasing for $x < x_1$, and, in case $D$ is bounded, strictly increasing for $x \geq \sup D$.
    To do this, we modify the definition of $f$ to be:
    $$f(x) = \begin{cases}
        x - x_1 + \epsilon_1 & ,x < x_1 \\
        \sum_{x_n \leq x} \epsilon_n & ,x \in D \\
        \sum_{x_n \leq x} \epsilon_n + (x - x_n)\frac{\epsilon_{n+1}}{x_{n+1} - x_n} & , x \in (x_n, x_{n+1}) \text{ for some } $n$ \\\
        \sum_{n=1}^{\infty} \epsilon_n + x - \sup D & , x \geq \sup D
    \end{cases},$$
    where we consider the last case to trivially never hold if $D$ is not bounded above.
    It's fairly easy to see that this function is indeed strictly increasing, since it is piecewise linear with a slope that is always positive, while it additionally maintains all the properties we previously proved (one has but to take limits at the points where the function's definition changes to see this).
\end{solution}