...
\begin{solution}
    ...
    Recall that $f(p) = p$ since $p$ is a fixed point for $f$, and observe that we have arrived at the same conclusion as in the previous exercise: if we choose $x_0 \in (p - \delta, p + \delta)$, it will always be the case that $\lvert f(x_0) - p \rvert < \lvert x_0 - p \rvert$, and this distance will monotonically shrink as we re-apply $f$, leading us to conclude that $f^{(n)}(x_0) \rightarrow p$, which means $p$ is indeed an attracting fixed point for $f$.
    Now suppose instead that $\lvert f'(p) \rvert > 1$.
    Again, for every $\epsilon > 0$ there exists $\delta > 0$ such that for $\lvert h \rvert < \delta$:

    \[\Bigl \lvert \frac{f(p + h) - f(p)}{h} - f'(p) \Bigr \rvert < \epsilon \implies \Biggl \lvert \Bigl \lvert \frac{f(p + h) - f(p)}{h} \Bigr \rvert - \lvert f'(p) \rvert \Biggr \rvert < \epsilon \implies \lvert f'(p) \rvert - \epsilon < \Bigl \lvert \frac{f(p + h) - f(p)}{h} \Bigr \rvert\]

    Now set $\epsilon = \lvert f'(p) \rvert - 1$, and obtain the $\delta$ for which:

    \[\lvert f'(p) \rvert - \lvert f'(p) \rvert + 1 < \Bigl \lvert \frac{f(p + h) - f(p)}{h} \Bigr \rvert \implies \lvert h \rvert < \lvert f(p + h) - f(p) \rvert\]

    By substituting $x = p + h$ we obtain $\lvert x - p \rvert < \lvert f(x) - p \rvert$ for all $x \in (p - \delta, p + \delta)$, which, just like the previous exercise, shows that if we start at any $x_0$ and compute $(f^{(n)}(x_0))$, whenever a term is in $(p - \delta, + \delta)$ the next one will \textit{not} be, and thus it can never be that $f^{(n)}(x_0) \rightarrow p$.
\end{solution}

\begin{exercise}{39}
    The cubic $x^3 - x - 1$ has a unique real root $x_0$ with $1 < x_0 < 2$.
    Find it!
    [Hint: Iterating the function $f(x) = x^3 - 1$ won't work! Why?]
\end{exercise}

\begin{solution}
    
    First, because $1^3 - 1 -1 = -1 < 0$ and $2^3 - 2 - 1 = 5 > 0$, by the intermediate value theorem it is indeed true that there exists a real root $x_0 \in (1, 2)$.
    Furthermore, $(x^3 - x - 1)' = 3x^2 - 1$, which is positive in $(1, 2)$, and so the polynomial is strictly increasing in $(1, 2)$, meaning that this root is unique.
    Let now $f(x) = x^3 - 1$.
    We observe that $f'(x) = 3x^2$, for which it holds that $3 < \lvert f'(x) \rvert < 12$ for $x \in [1, 2]$ (notice we use the closed interval such that our metric space is complete ---subsets of $\mathbb{R}$ are complete iff they are closed---).
    By exercise 37, this means that the fixed point $x_0 \in [1, 2]$ is a repelling one, and so we cannot approximate it by iterating $f^{(n)}(x_0)$ for any $x_0$.
    To deal with this, we consider the function $g(x) = \lambda(f(x) - x) + x$.
    If $\lambda \neq 0$ and $y$ is a fixed point for $g$, we have that $y = g(y) = \lambda(f(y) - y) + y \implies \lambda (f(y) - y) = 0 \implies f(y) = y$, and so we can find fixed points of $f$ by finding fixed points of $g$.
    We begin by computing:

    \[g'(x) = \lambda f'(x) - \lambda + 1 = 3\lambda x^2 - \lambda + 1\]

    In order for the fixed point of $g$ to be an attracting one, we want $\lvert g'(x) \rvert < 1$ in $(1, 2)$.
    We have $g''(x) = 6\lambda x$, and so for $\lambda < 0, g'(x)$ is strictly decreasing in $(1, 2)$.
    This means that for $x \in (1, 2), g'(1) > g'(x) > g'(2)$.
    We have $g'(1) = 3\lambda - \lambda + 1 = 2\lambda + 1, g'(2) = 12\lambda - \lambda + 1 = 11\lambda + 1$.
    We want these values to satisfy:

    \[g'(1) < 1 \implies 2\lambda + 1 < 1 \implies \lambda < 0, g'(2) > -1 \implies 11\lambda + 1 > -1 \implies \lambda > -2/11\]

    In order for $g$ to be usable it must also be the case that $g([1, 2]) \subset [1, 2]$.
    We observed that $g'(x)$ is strictly decreasing, and so its minimum value in $[1, 2]$ is $g'(2) = 12\lambda - \lambda + 1 = 11\lambda + 1$, which is positive for $\lambda > -1/11$.
    As a consequence, $g$ would then be strictly increasing, and so $g([1, 2]) = [g(1), g(2)] = [\lambda(f(1) - 1) + 1, \lambda(f(2) - 2) + 2] = [\lambda(0 - 1) + 1, \lambda(7 - 2) + 2] = [1 - \lambda, 5\lambda + 2]$.
    The constraints this yields are:

    \[1 - \lambda \geq 1 \implies 0 \geq \lambda, 5\lambda + 2 \leq 2 \implies \lambda \leq 0\]

    All in all, it suffices to pick $\lambda < 0, \lambda > -1/11$, so for example $\lambda = -1/10$ does the job, and we can find the fixed point of $g$ (and hence of $f$, and hence the root of $x^3 -x - 1$) by iterating over $g$.
\end{solution}

\begin{exercise}{40}
    Extend the result in Example 7.15 as follows:
    Suppose that $F: [a, b] \rightarrow \mathbb{R}$ is continuous on $[a, b]$, differentiable in $(a, b)$, and satisfies $F(a) < 0, F(b) > 0$, and $0 < K_1 \leq F'(x) \leq K_2$.
    Show that there is a unique solution to the equation $F(x) = 0$, and describe a methodology that is guaranteed to approximate it with arbitrary accuracy.
    [Hint: Consider the equation $f(x) = x$, where $f(x) = x - \lambda F(x)$ for some suitably chosen $\lambda$.]
\end{exercise}

\begin{solution}
    
    First, the fact that $F(a) < 0, F(b) > 0$ and that $F$ is continuous on $[a, b]$ allows us to use the intermediate value theorem to conclude that there exists at least one solution to $F(x) = 0, x \in (a, b)$.
    Secondly, $F'(x) > 0$ means that the function is monotonically increasing in $[a, b]$, and hence this solution is unique.
    In a generalization of the previous problem (39), we would like to solve this by finding a fixed point of $F(x) - x$.
    However, the same issues that were described above arise, since we don't know whether $\lvert F'(x) \rvert < 1$ in $(a, b)$.
    Consider the function $f(x) = x -\lambda F(x)$, and observe that for $\lambda \neq 0$, it holds that if $f(x) = x$, then $\lambda F(x) = 0 \implies F(x) = 0$.
    Therefore we can find roots of $F$ by finding fixed points of $f$.
    We have that:
    ...
    \[\lvert f'(x) \rvert < 1 \implies \lvert 1 - \lambda F'(x)  \rvert < 1 \implies -1 < 1 - \lambda F'(x) < 1 \implies \lambda F'(x) > 0, \lambda F'(x) < 2\]

    Since $F'(x) > 0$ for all $x \in (a, b)$, the first condition simplifies to $\lambda > 0$.
    In addition, from the second condition we obtain $\lambda < \frac{2}{F'(x)} \leq \frac{2}{K_1}$, since $K_1 \leq F'(x)$ for $x \in (a, b)$.
    In order for any of this to be applicable, it must be the case that $f([a, b]) \subset [a, b]$.
    The analysis here would be simplified if $f$ was strictly increasing, so let's examine what this would require:

    \[f'(x) > 0 \implies 1 > \lambda F'(x) \implies \lambda < \frac{1}{F'(x)} \leq \frac{1}{K_1}\]

    This is consistent with our constraints so far, since $0 < \frac{1}{K_1} < \frac{2}{K_1}$.
    We can thus assume that $\lambda$ is such that $f$ is strictly increasing, in which case:

    \[f([a, b]) = [f(a), f(b)] = [a - \lambda F(a), b - \lambda F(b)]\]

    We want $b - \lambda F(b) \leq b \implies 0 \leq \lambda F(b)$, which, because $F(b) > 0$, is always true for $\lambda > 0$.
    We also want $a - \lambda F(a) \geq a \implies \lambda F(a) \leq 0$, which, because $F(a) < 0$, is always true for $\lambda > 0$.
    Therefore, picking any $\lambda \in (0, \frac{1}{K_1})$ results in $f$ having a unique and attracting fixed point in $(a, b)$, and so iterative applications of $f$ will always converge to it starting from any initial ``guess'' in $(a, b)$.
    As we observed, this fixed point $p$ will satisfy $F(p) = 0$.

\end{solution}

\begin{exercise}{41}
    Let $M$ be complete and let $f: M \rightarrow M$ be continuous.
    If $f^{(k)}$ is a strict contraction for some integer $k > 1$, show that $f$ has a unique fixed point.
\end{exercise}

\begin{solution}
    
    By Theorem 7.13 we know that $f^{(k)}$ has a unique fixed point $p$.
    We first show that if $f$ has a fixed point, it must be unique.
    Indeed, suppose $p' \in M$ is such that $f(p') = p'$.
    This means of course that $f^{(k)}(p') = p'$, and so $p'$ is a fixed point of $f^{(k)}$, which has a unique fixed point.
    We've therefore more specifically shown that if $f$ has a fixed point, this must be $p$.
    Furthermore, we know that for any $x_0 \in M$, it is the case that $\lim_{n \rightarrow \infty} (f^{(k)})^{(n)}(x_0) = p$.
    This can be thought of as a sequence of the form:

    \[f^{(k)}(x_0), f^{(2k)}(x_0), f^{(3k)}(x_0), \ldots\]

    Consider the sequence $f^{(n)}(x_0)$.
    For any $n$, by the division theorem for integers we know that there exist unique $q, r \in \mathbb{N}$ such that $n = qk + r, r \in \{0, 1, \ldots, k - 1\}$.
    Therefore, we have that:

    \[d(f^{(n)}(x_0), p) = d(f^{(qk + r)}(x_0), p) \leq d(f^{(qk)}(f^{(r)}(x_0)), f^{(qk)}(x_0)) + d(f^{(qk)}(x_0), p)\]
    \[\leq a^{q}d(f^{(r)}(x_0), x_0) + d(f^{(qk)}(x_0), p),\]

    where $a$ is the contraction factor for $f^{(k)}$.
    Notice that the second term of the RHS here can be made arbitrarily small  after some $N_1$ due to $\lim_{n \rightarrow \infty}(f^{(k)})^{(n)}(x_0) = p$.
    Furthermore, notice that $r$ can only take $k$ different values, and so we can define:
    
    \[M = \max_{r \in \{0, 1, \ldots, k - 1\}}\{d(f^{(r)}(x_0), x_0)\},\]

    leading to $d(f^{(n)}(x_0), p) \leq a^qM + d(f^{(qk)}(x_0), p)$.
    Notice then that because $a < 1$, by selecting $n \geq N_2$ large enough so that $q$ also grows large enough, the first term of the RHS above can \textit{also} be made arbitrarily small.
    This leads us to conclude that for any given $\epsilon > 0$, after some sufficiently large $N = \max\{N_1, N_2\}$, for all $n \geq N$ it holds that $d(f^{(n)}(x_0), p) < \epsilon$.
    Therefore:

    \[\lim_{n \rightarrow \infty} f^{(n)}(x_0) = p \implies f(p) = \lim_{n \rightarrow \infty} f^{(n + 1)}(x_0) = \lim_{n \rightarrow \infty} f^{(n)}(x_0) = p,\]

    where we have used the continuity of $f$.
    We thus conclude that $p$ is indeed a fixed point of $f$.
...
\begin{exercise}{43}
    ...
    (b) $f$ satisfies only $d(f(x), f(y)) < d(x, y)$ for all $x \neq y$ (but $M$ is still complete)
\end{exercise}

\begin{solution}
    
    (a) Let $M = (0, 1]$, which is incomplete due to the fact that we can find Cauchy sequences inside $M$ that converge to $0 \notin M$.
    Let also $f(x) = \frac{x^2}{4}$, for which $f'(x) = \frac{x}{2}$.
    It's clear that $f((0, 1]) = (0, 1/4] \subset (0, 1]$.
    Furthermore, since $\lvert f'(x) \rvert < 1$ in $(0, 1)$, $f$ is a strict contraction.
    To find fixed points of $f$ we have that:

    \[f(x) = x \implies \frac{x^2}{4} = x \implies x^2 - 4x = 0 \implies x(x - 4) = 0 \implies x = 0 \text{ or } x = 4\]

    Neither of these points are in $M$, and thus $f$ has no fixed points when considered as a map from $M$ to $M$.

    (b) Consider the metric space $M = [0, \infty)$, which is complete as a closed subset of $\mathbb{R}$.
    Let $f:M \rightarrow \mathbb{R}, f(x) = x + \frac{1}{x + 1}$, which has $f(x) > 0$ whenever $x \geq 0$, and so we can think of it as $f: M \rightarrow M$.
    Notice that $f(x) = x \implies \frac{1}{x + 1} = 0$, which has no solutions, and hence $f$ has no fixed points.
    Pick any two $x, y$ with $x < y$. We have that $d(x, y) = y - x$.
    Furthermore:

    \[d(f(x), f(y)) = \Bigl \lvert x + \frac{1}{x + 1} - y - \frac{1}{y + 1}\Bigr \rvert = \Bigl \lvert x - y + \frac{y + 1 - x - 1}{(x + 1)(y + 1)}\Bigr \rvert = \Bigl \lvert (y - x)(-1 + \frac{1}{(x + 1)(y + 1)}) \Bigr \rvert\]

    \[= d(x, y) \Bigl \lvert 1 - \frac{1}{(x + 1)(y + 1)} \Bigr \rvert\]

    Notice that the factor that multiplies $d(x, y)$ here is always strictly less than 1, since $x, y \geq 0$, and so indeed $d(f(x), f(y)) < d(x, y)$.
    Nevertheless, by making $x, y$ increasingly large, we can make this factor arbitrarily close to 1, meaning that no constant $a < 1$ would satisfy $d(f(x), f(y)) \leq ad(x, y)$ for all $x, y, x \neq y$.
\end{solution}
...
\begin{exercise}{46}
    ...
    [Hint: If $\hat{M}$ is the completion for $M$, then $A$ is dense in $\hat{M}$.
    Why?]
\end{exercise}

\begin{solution}
    
    Since so far in the book we haven't worked with the strict definition (i.e., we've assumed for simplicity that a set is a subset of its completion), we will do this here for the sake of working through all the details at least once.
    Suppose then that $\hat{M}$ is a/the completion for $M$.
    This means that $M$ is isometric to a dense subset of $\hat{M}$, in other words that there exists a function $F: M \rightarrow \hat{M}$ that is an \textit{into} isometry, and is such that $F(M)$ is dense in $\hat{M}$.
    Clearly, the function $F\vert_A: A \rightarrow \hat{M}, F\vert_A(x) = F(x)$ is also an isometry.
    The question is whether $F\vert_A(A)$ is dense in $\hat{M}$.
    Let therefore $y \in \hat{M}$.
    Because $F(M)$ is dense in $\hat{M}$, there exists $(y_n) \subset F(M)$ such that $y_n \rightarrow y$.
    For any $\epsilon > 0$, we can thus find $N$ such that for $n \geq N, \hat{d}(y_n, y) < \epsilon/2$.
    Now, there exist $x_n \in M$ such that $F(x_n) = y_n$.
    Because $A$ is dense in $M$, there exists $(z_n) \subset A$ such that $z_n \rightarrow x_N$.
    Pick $K > 0$ such that $d(z_n, x_N) < \epsilon/2$ for all $n \geq K$.
    We observe that, for any $n \geq K$:

    \[\hat{d}(F(z_n), y) \leq \hat{d}(F(z_n), y_N) + \hat{d}(y_N, y) = d(z_n, x_N) + \hat{d}(y_N, y) < \epsilon/2 + \epsilon/2 = \epsilon\]

    If, then, for increasingly small $\epsilon$ (e.g., $1/n$) we pick $z_n \in A$ as described above, we know that the sequence $F(z_n) \subset F\vert_A(A)$ converges to $y$, which means precisely that $F\vert_A(A)$ is dense in $\hat{M}$, and so $A, M$ indeed have the same completion isometrically.
\end{solution}