...
\begin{solution}
    ...
    We need to find an appropriate $\epsilon' > 0$ such that $B_{\epsilon'}^{d}(x) \subset S$.
    As a reminder from exercise 3.10, the metric that is used in the Hilbert cube $H^{\infty}$ is $d(x, y) = \sum_{n=1}^{\infty}  2^{-n}\lvert x_n - y_n \rvert$.
    Pick $\epsilon' = \frac{\epsilon}{2}$.
    Then the corresponding ball around $x$ is:

    $$B_{\epsilon'}^{d}(x) = \{z \in H^{\infty}: \sum_{n=1}^{\infty} 2^{-n}\lvert z_n - x_n \rvert < \epsilon/2\}$$
\end{solution}
