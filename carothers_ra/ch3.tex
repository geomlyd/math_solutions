...
\begin{solution}
    ...
    $$d(x, y) \leq d(x_n, y) + d(x_n, x) \implies d(x_n, y) \geq d(x, y) - d(x_n, x) > d(x, y) - \epsilon \implies d(x_n, y) - d(x, y) > -\epsilon$$
...
\begin{solution}
    ...
    Because $(x_{n_k})$ contains infinite terms of $(x_n)$ selected in an order which maintains the order of indices, it must be the case that there exists $K > 0$ such that for all $k \geq K, n_k \geq N$, which in turn means that from that point on, $d(x_{n_k}, x) < \epsilon$.
