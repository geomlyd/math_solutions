...
\begin{solution}
    ...
    Suppose $(x_n, y_n)$ converges to $(x, y)$  under $d_1$.
    Pick any $\epsilon > 0$ and set $\epsilon' = \epsilon / \sqrt{2}$.
    By the property just proved above, we can find $N, M$ such that $d(x_n, x) < \epsilon', \rho(y_n, y) < \epsilon'$.
    For $K = \max\{N, M\}$ and $n \geq K$:

    $$d_2((x_n, y_n), (x, y)) = \sqrt{d(x_n, x)^2 + \rho(y_n, y)^2} < \sqrt{\epsilon'^2 + \epsilon'^2} = \sqrt{\epsilon^2/2 + \epsilon^2/2} = \epsilon,$$

    which shows convergence under $d_2$.
\end{solution}