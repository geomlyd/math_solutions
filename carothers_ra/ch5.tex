...
\begin{solution}
    ...
    Notice, first that since the set of $x_n$ is dense, we can find one such $x_i$ such that $\rho(x, x_i) < \epsilon/4$.
    Now, based on this $i$, suppose that $\delta = \epsilon/2^{i+2}$ and that $y$ is such that we have $d(f(x), f(y)) < \delta$.
    Then:

    \[ d(f(x), f(y)) < \delta \implies \sum_{n=1}^{\infty} 2^{-n} \lvert \rho(x, x_n) - \rho(y, x_n) \rvert < \delta \implies 2^{-i} \lvert \rho(x, x_i) - \rho(y, x_i) \rvert < \delta, \]

    where we separated the term corresponding to the $x_i$ we picked above.
    Thus:

    \[\lvert \rho(x, x_i) - \rho(y, x_i) \rvert < \delta 2^{i} = \epsilon/4 \implies \rho(y, x_i) < \rho(x, x_i) + \epsilon/4 = \epsilon/2\]

    Now we have that $\rho(x, y) < \rho(x, x_i) + \rho(y, x_i) = \epsilon/4 + \epsilon/2 = \epsilon \cdot 3/4 < \epsilon$, and so we have effectively shown that $f^{-1}$ (which is well-defined when thinking about it as a function inside the range of $f$ and \textit{not} all of $H^{\infty}$), thus showing that $f$ is a homeomorphism into $H^{\infty}$.
\end{solution}
...
\begin{solution}
    ...
    For any $x \in [a, b], \lvert f(x) \rvert \leq \max_{a \leq t \leq b} \lvert f(t) \rvert = \lvert \lvert f \rvert \rvert_{\infty}$, and similarly $\lvert g(x) \rvert < \lvert \lvert g \rvert \rvert_{\infty}$.
    We thus obtain $\lvert f(x) g(x) \rvert \leq \lvert \lvert f \rvert \rvert_{\infty} \cdot \lvert \lvert g \rvert \rvert_{\infty}$ and:

    \[\lvert \lvert f g \rvert \rvert_{\infty} = \max_{a \leq t \leq b} \lvert f(t) g(t) \rvert \leq \lvert \lvert f \rvert \rvert_{\infty} \cdot \lvert \lvert g \rvert \rvert_{\infty}\]

    We also have that:

    \[ \lvert \lvert \max\{f, g\} \rvert \rvert_{\infty} = \max_{a \leq t \leq b} \lvert \max\{f, g\}(t) \rvert = \max_{a \leq t \leq b} \lvert \max\{f(t), g(t)\} \rvert \leq \max\{\lvert \lvert f \rvert \rvert_{\infty}, \lvert \lvert g \rvert \rvert_{\infty}\},\]

    since every value of $f, g$ is smaller than the corresponding maximum value of the function.

    Lastly, suppose $\lvert f \rvert \leq \lvert g \rvert$.
    This means that for all $t \in [a, b], \lvert f(t) \rvert \leq g(t) \rvert$.
    Consequently, the same inequality must hold for their maximum values, which means $\lvert \lvert f \rvert \rvert_{\infty} \leq \lvert \lvert g \rvert \rvert_{\infty}$.
\end{solution}

\begin{exercise}{64}
    Given $n \in \mathbb{N}$ and $f, g \in C(\mathbb{R})$, let $d_n(f, g) = \max_{\lvert t \rvert \leq n} \lvert f(t) - g(t) \rvert$.
    Then $d_n$ defines a pseudometric on $C(\mathbb{R})$ (Why?) Show that $d(f, g) = \sum_{n=1}^{\infty}2^{-n} d_n(f, g)/(1 + d_n(f, g))$ defines a metric on $C(\mathbb{R})$.
\end{exercise}

\begin{solution}
    
    To show that $d_n$ is a pseudometric on $C(\mathbb{R})$, we have the following:

    \begin{itemize}
        \item From its definition, it is obvious that $d_n(f, g) \geq 0$ for any two $f, g \in C(\mathbb{R})$.
        \item Again from the definition, $d_n(f, g) = d_n(g, f)$ for any two $f, g \in C(\mathbb{R})$. 
        \item If $f = g$, then clearly $d_n(f, g) = 0$.
        \textit{However}, what makes $d_n$ a pseudometric instead of a metric is the fact that for e.g. $f(t) = t, t \leq n, f(t) = 2(t - n) + n$ and $g(t) = t, t \leq n, g(t) = 3(t - n) + n$ (both clearly continuous, and not equal functions), $d_n(f, g) = 0$.
        \item For $f, g, h \in C(\mathbb{R})$, we have that:
        
        \[d_n(f, h) = \max_{\lvert t \rvert \leq n} \lvert f(t) - h(t) \rvert = \max_{\lvert t \rvert \leq n} \lvert f(t) - g(t) + g(t) - h(t) \rvert\]

        Since it is the case that $\lvert f(t) - g(t) + g(t) - h(t) \rvert \leq \lvert f(t) - g(t) \rvert + \lvert g(t) - h(t) \rvert$, the inequality must also hold when applying $\max_{\lvert t \rvert \leq n}$ to both sides.
        Furthermore, the maximum of a sum equals at most the sum of the two maxima, and so we obtain:

        \[d_n(f, h) \leq \max_{t \leq n} \lvert f(t) - g(t) \rvert + \max_{t \leq n} \lvert g(t) - h(t) \rvert = d_n(f, g) + d_n(g, h),\]

        which is the triangle inequality for $d_n$.
    \end{itemize}
    Now we examine $d$ in the same manner:
    \begin{itemize}
        \item Due to the definition of this metric as an infinite sum, we need to first determine whether it is always well-defined, i.e.\ whether the sum always converges.
        To see this, observe that $d_n(f, g)/(1 + d_n(f, g)) \leq 1$, and so each partial sum of $d$ is bounded above by the geometric series $\sum_{n=1}^{\infty} 2^{-n}$, which we know converges.
        Since the partial sums are also non-decreasing, $d(f, g)$ does in fact always converge.
        \item By the non-negativity of each $d_n$, it is clear that $d(f, g) \geq 0$ always holds as well.
        \item Similarly, by the symmetry of each $d_n$, $d(f, g) = d(g, f)$.
        \item Clearly, $d(f, f) = 0$ for any $f \in C(\mathbb{R})$.
        Now pick any two $f, g \in C(\mathbb{R}), f \neq g$.
        Then there exists $x \in \mathbb{R}$ such that $f(x) \neq g(x)$.
        This means that for $n = \lceil x \rceil, d_n(f, g) = \max_{\lvert t \rvert \leq n} \lvert f(t) - g(t) \rvert > 0$, and so $d(f, g) > 0$ as well, thus showing that $d(f, g) = 0$ iff $f = g$.
        \item For the triangle inequality, we use Exercise 7 of Chapter 3, by observing that each term of the sum of $d$ equals $2^{-n}\sigma(d_n(f, g))$ (for the $\sigma$ of Exercise 6 of that Chapter).
        Although $d_n$ is not a metric, the proof that for some metric $d$, $\sigma(d(x, y))$ satisfies the triangle inequality if $\sigma(x + y) \leq \sigma(x) + \sigma(y)$ only depended on $d$ satisfying the triangle inequality, which is the case for $d_n$.
        Since the triangle inequality is satisfied for each term of the convergent sum $d$, it also holds for $d$ itself, thus completing the proof that $d$ is a metric.
    \end{itemize}
\end{solution}