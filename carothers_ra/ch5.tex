...
\begin{solution}
    ...
    In the other case, we have that there exists a sequence $(x_n)$ such that $x_n \rightarrow x$.
    Let $A = (x_n)$.
    The uniqueness of limits along with the fact that for any set $C$ it holds that $\overline{C}$ contains precisely the points to which a sequence in $C$ converges shows that $\overline{A} = (x_n) \cup \{x\}$.
    Then we must have that:
    
    \[f(\overline{A}) = f((x_n) \cup \{x\}) \subset \overline{f(A)} = \overline{f((x_n))}\]

    More specifically, this means that $f(x)$ belongs in the closure of $f((x_n))$, and thus that there exists a sequence $f(x_i) \rightarrow f(x)$
\end{solution}