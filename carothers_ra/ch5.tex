...
\begin{solution}
    ...
    \[N = \bigcup_{i=1}^{\infty} I_n, I_n = (a_n, b_n),\]

    where $I_n \cap I_m = \emptyset$ for $n \neq m$.
    Then we have that $f^{-1}(N) = \bigcup_{i=1}^{n} f^{-1}(I_n)$.
    For any such $I_n$, write $I_n = (\infty, b_n) \cap (a_n, \infty)$.
    If one of the boundaries of $I_n$ is already one of the infinities, then leave it as is.
