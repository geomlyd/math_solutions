...
\begin{solution}
    ...
    First, recall that for any closed set $K, K = \partial K \cup \mathring{K}$.
    ...
    \[ g(z) = g(z_1, \ldots, z_n) = \sum_{i=1}^{p} (\Phi_i(z_1, \ldots, z_n) - y_i)^2 \implies \frac{\partial g}{\partial x_j}(z)(h) = \sum_{i=1}^{p} 2(\Phi_i(z_1, \ldots, z_n) - y_i)\frac{\partial \Phi_i}{\partial x_j}(z)(h),\]

    where $h \in \mathbb{R}$ (since $\frac{\partial g}{\partial x_j}(z) \in \mathcal{L}(\mathbb{R}, \mathbb{R})$).
    Notice that by recalling that $D_j \Phi(x) = \begin{pmatrix}
        \frac{\partial \Phi_1}{\partial x_j}(x) \\
        \vdots \\
        \frac{\partial \Phi_n}{\partial x_j}(x)
    \end{pmatrix} \in \mathcal{L}(\mathbb{R}, \mathbb{R}^p)$, we can rewrite the above as:

    \[\frac{\partial g}{\partial x_j}(z)(h) = 2 \langle D_j \Phi(z)(h), \Phi(z) - y \rangle = 0\]

    Notice that this holds for all $j = 1, \ldots, n$.
    If we sum all of the resulting equations, we obtain that:

    \[\sum_{j=1}^{n} D_j \Phi(z)(\Phi(z) - y) = 0\]

    Now recall that we are given that $D \Phi(x)$ is surjective for any $x$.
    More specifically, $D \Phi(z)$ is surjective, which means that for $w = \Phi()
\end{solution}
