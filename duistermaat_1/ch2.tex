...
\begin{exercise}{ - Unlisted; Another proof of theorem 2.5.3 (Multivariate Mean Value Theorem)}
    Let $U$ be a convex open subset of $\mathbb{R}^n$ and let $f: U \rightarrow \mathbb{R}^p$ be a differentiable mapping.
    Suppose that the derivative $Df: U \rightarrow \mathcal{L}(\mathbb{R}^n, \mathbb{R}^p)$ is bounded on $U$, that is, there exists $k > 0$ with $\lvert \lvert Df(\xi)(h) \rvert \rvert \leq k \lvert \lvert h \rvert \rvert$, for all $\xi \in U$ and $h \in \mathbb{R}^n$, which is the case if $\lvert \lvert Df(\xi)\rvert \rvert \leq k$.
...
\begin{solution}
    ...
    Fix any two $x, y \in \mathbb{R}^n$.
    The statement is obvious if $f(x) = f(y)$, so from now on assume they are not equal.
    Define the following function $g: U \rightarrow \mathbb{R}$:

    $$g(z) = \langle f(y) - f(x), f(z) \rangle$$

    By the known theorem regarding differentiation of inner products, we know that $g$ is differentiable with:
    
    $$Dg(z)(h) = \langle f(y) - f(x), Df(z)(h) \rangle$$

    Notice, also, that due to the convexity of $U$ we can apply the mean value theorem on the segment connecting $x, y$ to obtain the existence of a $c \in U$ on that segment such that:

    $$g(y) - g(x) = Dg(c)(y-x) = \langle f(y) - f(x), Df(c)(y - x) \rangle \implies $$
    $$\langle f(y) - f(x), f(y) \rangle - \langle f(y) - f(x), f(x) \rangle = \langle f(y) - f(x), Df(c)(y - x) \rangle \implies$$
    $$\lvert \lvert f(y) - f(x) \rvert \rvert^2 = \langle f(y) - f(x), Df(c)(y-x) \rangle$$

    By using the Cauchy-Schwarz inequality:

    $$\lvert \lvert f(y) - f(x) \rvert \rvert^2 \leq \lvert \lvert f(y) - f(x) \rvert \rvert \cdot \lvert \lvert Df(c)(y - x) \rvert \rvert \implies \lvert \lvert f(y) - f(x) \rvert \rvert \leq \lvert \lvert Df(c)(y-x) \rvert \rvert \leq k \lvert \lvert y - x \rvert \rvert,$$
...
\begin{exercise}{26}
    Let $U \subset \mathbb{R}^n$ be convex and open and let $f:U \rightarrow \mathbb{R}^n$ be differentiable.
    Assume that the self-adjoint part of $Df(x)$ is positive-definite for each $x \in U$, that is, $\langle h, Df(x)(h) \rangle > 0$ if $0 \neq h \in \mathbb{R}^n$.
    Prove that $f$ is injective.

    \textbf{Hint}: Suppose $f(x) = f(x')$, where $x' = x + h$.
    Consider $g: [0, 1] \rightarrow \mathbb{R}$ with $g(t) = \langle h, f(x + th) \rangle$.
    Note that $g(0) = g(1)$ and deduce $g'(\tau) = 0$ for some $0 < \tau < 1$.

    \textbf{Background}: For $n = 1$, this is the well-known result that a function $f: I \rightarrow \mathbb{R}$ on an interval $i$ is strictly increasing and hence injective if $f' > 0$ on $I$.
\end{exercise}
...
\begin{solution}
    ...
    $$Df(x) = \frac{Df(x) + Df(x)^*}{2} + \frac{Df(x) - Df(x)^*}{2},$$

    and the ``self-adjoint part'' of $Df(x)$ is the operator $\frac{Df(x) + Df(x)^*}{2}$.
    It is given that this operator is positive-definite (since it is by definition self-adjoint) for each $x \in U$, which means:

    $$\langle h, \Bigl (\frac{Df(x) + Df(x)^*}{2} \Bigr )(h) \rangle = \frac{1}{2}\langle h, Df(x)(h) \rangle + \frac{1}{2}\langle h, Df(x)^*(h) \rangle = \frac{1}{2}\langle h, Df(x)(h) \rangle + \frac{1}{2}\langle Df(x)(h), h \rangle$$
    $$ = \langle h, Df(x)(h) \rangle,$$

    and so we can conclude that $\langle h, Df(x)(h) \rangle > 0$ for any $x, h, h \neq 0$.
    Motivated by the hint, assume that $f$ is not injective, and thus that $f(x) = f(x')$ for some $x \neq x'$, and let $x' = x + h$.
    Define $g$ as in the hint, and observe that this is a differentiable function as well.
    It is the case that $g(0) = \langle h, f(x) \rangle, g(1) = \langle h, f(x + h) \rangle = \langle h, f(x') \rangle = \langle h, f(x) \rangle = g(0)$ and so by the mean value theorem we conclude that there exists $\tau \in (0, 1)$ such that $g'(\tau) = 0$.
    Then:

    $$g'(\tau) = 0 \implies (\sum_{i=1}^{n} h_i f_i(x + \tau h))' = 0 \implies \sum_{i=1}^{n} h_i Df_i(x + \tau h)(h) = 0 \implies \langle h, Df(x'')(h) \rangle = 0,$$

    where $x'' = x + \tau h$.
    Clearly, this contradicts $\langle h, Df(x)(h) \rangle > 0$ for all $x, h, h \neq 0$, and so $f$ must indeed be injective.
\end{solution}

\begin{exercise}{28}
    Let $f: \mathbb{R}^n \rightarrow \mathbb{R}^n$ be a differentiable mapping and assume that
    
    $$\sup\{\lvert \lvert Df(x) \rvert \rvert_{\text{Eucl}} \lvert x \in \mathbb{R}^n\} \leq \epsilon < 1$$

    Prove that $f$ is a contraction with contraction factor $\leq \epsilon$.
    Suppose $F \subset \mathbb{R}^n$ is a closed subset satisfying $f(F) \subset F$.
    Show that $f$ has a unique fixed point $x \in F$.
\end{exercise}

\begin{solution}
    
    We first note that the hypothesis guarantees that for any $x \in \mathbb{R}^n, \lvert \lvert Df(x) \rvert \rvert_{\text{Eucl}} \leq \epsilon$.
    Recall that the mean value theorem (2.5.3 in the book) guarantees that $f$ is Lipschitz continuous on $\mathbb{R}^n$ with Lipschitz constant $\epsilon$, which is to say that:

    $$\lvert \lvert f(x) - f(x') \rvert \rvert \leq \epsilon \lvert \lvert x - x' \rvert \rvert,$$

    for any two $x, x' \in \mathbb{R}^n$.
    Because $\epsilon < 1$, we immediately obtain that $f$ is a contraction with contraction factor $\leq \epsilon$.
    The second part of the exercise follows immediately from the contraction lemma (1.7.2).    
\end{solution}
...
\begin{solution}
    ...
    If $F: \mathbb{R}^n \rightarrow \mathbb{R}$ is differentiable and $x_0 = (x_{01}, x_{02}, \ldots, x_{0n})$ is a local extremum of $F$, then $DF(x_0) = 0$.

    Indeed, pick any $i = 1, 2, \ldots, n$, and notice that the function $g(x_i) = F(x_{01}, \ldots, x_i, \ldots, x_{0n})$ must have a local extremum at $x = x_{0i}$: if this were not the case, by perturbing the $i$-coordinate of $x_0$ we would have that $F$ does not assume a locally extreme value at $x_0$ either.
    Fermat's theorem then guarantees that $g'(x_i) = 0 \implies D_i F(x_0) = 0$.
    The fact that this holds for all $i$ shows that $DF(x_0) = 0$.

    Now, for the exercise, recall that for any closed set $K, K = \partial K \cup \mathring{K}$.
    In our case here, we know that all points on the boundary of $K$ satisfy $f(x) \geq \delta$.
    Since there exists \textit{at least one} $x \in K$ such that this does not hold (i.e., $f(x) < \delta$ for some $x \in K$), we therefore conclude that for this $x, x \notin \partial K$, which, from the above equality, implies $x \in \mathring{K}$.
    Therefore $K$ has a non-empty interior.
    Consider the function $g: \mathbb{R}^n \rightarrow \mathbb{R}, g(x) = f^2(x)$, which is of course differentiable and thus continuous.
    We know then that it must achieve a minimum value on the compact set $K$.
    The fact that $f(x) \geq \delta$ for all boundary points of $K$, and that there exists $z \in K$ with $f(z) < \delta$ implies that this minimum value must necessarily be achieved in the interior of $K$.

    Suppose then it is achieved on $z \in \mathring{K}$.
    From the lemma above, we have that it must hold that $Dg(z) = 0$.
    Let $\Phi(x) = (\Phi_1(x), \ldots, \Phi_p(x))$ where $\Phi_i: \mathbb{R}^n \rightarrow \mathbb{R}$ and then we have that:

    \[ g(z) = g(z_1, \ldots, z_n) = \sum_{i=1}^{p} (\Phi_i(z_1, \ldots, z_n) - y_i)^2 \implies \frac{\partial g}{\partial x_j}(z) = \sum_{i=1}^{p} 2(\Phi_i(z_1, \ldots, z_n) - y_i)\frac{\partial \Phi_i}{\partial x_j}(z)\]

    Notice that by recalling that $D_j \Phi(x) = \begin{pmatrix}
        \frac{\partial \Phi_1}{\partial x_j}(x) \\
        \vdots \\
        \frac{\partial \Phi_n}{\partial x_j}(x)
    \end{pmatrix} \in \mathcal{L}(\mathbb{R}, \mathbb{R}^p)$, and by multiplying both sides with any non-zero $h_1$, we can rewrite the above as:

    \[\frac{\partial g}{\partial x_j}(z)h_1 = 2 \langle D_j \Phi(z)h_1, \Phi(z) - y \rangle\]

    Notice that this must be zero for any $h_1 \neq 0$.
    Furthermore, by  applying this procedure to all $j$, each time with a different variable $h_j \neq 0$, and by summing the equations, we obtain that:

    \[2\langle \sum_{j=1}^{n} D_j \Phi(z)h_j, \Phi(z) - y \rangle = 0 \implies 2 \langle D \Phi(z)(h), \Phi(z) - y \rangle = 0,\]

    where $h = (h_1, \ldots, h_n)$ and where we think about matrix multiplication in terms of its columns.
    Now recall that we are given that $D \Phi(x)$ is surjective for any $x$.
    More specifically, $D \Phi(z)$ is surjective, which means that for $w = \Phi(z) - y \in \mathbb{R}^p$, there must exist some $h \in \mathbb{R}^n$ such that $D\Phi(z)(h) = w$.
    Substituting in the above equality, which must hold for any $h$:

    \[2 \langle \Phi(z) - y, \Phi(z) - y \rangle = 0,\]

    which implies that $\Phi(z) = y$.
\end{solution}
